...
Sub Init5()
    ...
    MakeOpcode 5, &H43, 2, 0, "VarLateMemSt"
    MakeOpcode 5, &H44, 4, 0, "VarLateMemCallSt"
    MakeOpcode 5, &H45, 4, 0, "VarLateMemStAd"
...
Function File16&(addr&)
    Dim t%
    CopyMemory t%, File(addr), 2
    File16 = t
End Function

Function File32&(addr&)
On Error Resume Next
    Dim t&
...
On Error Resume Next
    ...
    If InStr(RefName(a), Hex(src)) = 0 Then
        If RefName(a&) <> "" Then RefName(a&) = RefName(a&) + ", "
...
Sub ReadOpcode(f$, o() As OpcodeType, index&)
    Dim t$, i&, c&
...
Function FastName$(prefix$, addr&)
    If addr >= Base And addr < Base + PESize Then
        If SubName(addr&) > "" Then
